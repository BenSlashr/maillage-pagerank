...
from app.services.google_auth_session import (
...
async def google_auth(request: Request):
    ...
    try:
        ...
        auth_url, state = get_auth_url(request)
...
async def google_callback(request: Request, code: str = Query(...), state: str = Query(...)):
    ...
    try:
        ...
        token_data = exchange_code(request, code, state)
        
        # S'assurer que le token est bien enregistré dans la session
...
async def google_analytics(request: Request, analytics_request: GoogleAnalyticsRequest):
    ...
    try:
        ...
        formatted_data = format_search_analytics_to_excel(analytics_data, analytics_request.dimensions)
